...
DESC_STATS <-
  as_tibble(cbind(
    ...
    annualized_return = ((1 + mean(d_temp$ret_port))^(1/14) - 1)*100,
    annualized_risk = sd(d_temp$ret_port) * sqrt(12),
...
financials_spx <- read.csv("SP500_Historical_Monthly_Final.csv", header = TRUE) %>%
  mutate(Date = ymd(Date)) %>%
  mutate(Date = floor_date(Date, "month")) %>%
  filter(Date >= Start_Date & Date <= End_Date) %>%
  drop_na(Date)
financials_stoxx <- read.csv("STOXX600_Historical_Monthly_Final.csv", header = TRUE) %>%
  mutate(Date = ymd(Date)) %>%
  mutate(Date = floor_date(Date, "month")) %>%
  filter(Date >= Start_Date & Date <= End_Date) %>%
  drop_na(Date)
...
processing <- sample_input %>%
  filter(TRBC.Industry.Name == "Fishing & Farming" |
           ...
           TRBC.Industry.Name == "Brewers" |
           TRBC.Industry.Name == "Distillers & Wineries" |
...
gafs_port_list <- vector(mode = "list", length = 4)
...
divest_port_list_spx[[5]] <- sample_input_spx
...
divest_port_list_stoxx[[5]] <- sample_input_stoxx
...
INF_RESULTS_SPX <- inferential_statistics(divest_port_list_spx[[1]])
